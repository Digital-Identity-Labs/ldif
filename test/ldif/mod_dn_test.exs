defmodule ModDNTest do
  ...
  describe "apply_to/2" do
        ...
        test "returns the Entry unchanged if the passed entry does not match the DN" do
          ...
          assert [%Entry{dn: @dn}] = ModDN.apply_to(change, %Entry{dn: @dn})
        end
        ...
        test "returns an entry with an adjusted rdn if set to do so" do
          ...
          assert [%Entry{dn: "ou=Product Development Accountants, ou=Product Development, dc=airius, dc=com"}] = ModDN.apply_to(change, %Entry{dn: @dn})
        end
        ...
        test "returns an entry with an adjusted superior if set to do so" do
          ...
          assert [%Entry{dn: "ou=PD Accountants, ou=Accounting, dc=airius, dc=com"}] = ModDN.apply_to(change, %Entry{dn: @dn})
        ...
        test "returns an adjusted entry and the original in a list if set to do so" do
          ...
          %Entry{dn: @dn},
          ] = ModDN.apply_to(change, %Entry{dn: @dn})
        ...
        test "both rnd and superior can be changed together" do
          ...
          assert [%Entry{dn: "ou=Product Development Accountants, ou=Accounting, dc=airius, dc=com"}] = ModDN.apply_to(change, %Entry{dn: @dn})
